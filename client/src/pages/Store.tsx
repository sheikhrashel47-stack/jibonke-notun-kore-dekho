...
export default function Store() {
  ...
  const featured = bookDefinitions.knowledge;
  ...
  return <section className="jibon-surface jibon-store">
    ...
    <section className="jibon-store__collection"><header><span className="jibon-kicker">পছন্দ করে পড়ো</span><h2>আজ কোন দিকে মন যাচ্ছে?</h2><p>প্রতিটি বই নিজের মতো করে শুরু করা যায়। তোমার বর্তমান প্রশ্নটাকেই পথ দেখাতে দাও।</p></header><div className="jibon-category-row" aria-label="বিষয়"><span>মন</span><span>জীবন</span><span>ব্যক্তিগত উন্নতি</span><span>আচরণ ও জীবনযাপন</span><span>মনোবিজ্ঞান</span><span>সীরাত ও ইতিহাস</span><span>চিন্তা</span><span>মস্তিষ্ক ও শেখা</span><span>অর্থ ও সম্পদ</span><span>বুদ্ধিমত্তা</span><span>সাধারণ জ্ঞান</span><span>বাস্তববাদী জীবন</span><span>সাহিত্যিক উপন্যাস</span></div><div className="premium-book-grid">{bookIds.map((id) => { const book = bookDefinitions[id]; return <PremiumBookCard key={id} book={book} progress={getBookProgress(id, progress)} chapterId={getCurrentChapter(id, lastChapterByBook)} inLibrary={libraryBookIds.includes(id)} onLibraryToggle={() => setLibraryBook(id, !libraryBookIds.includes(id))} />; })}</div></section>
  </section>;
}
