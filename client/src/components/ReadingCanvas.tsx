/* জীবন-ড্যাশবোর্ড: chapter reader-এ web text, visible page-turn mode ও স্পষ্ট reading flow দেখায়। */
...
import { ArrowLeft, ArrowRight, CheckCircle2, Clock3, FileText, Hand, Highlighter, Quote, ScrollText, X } from "lucide-react";
import { useEffect, useState } from "react";
import { Link } from "wouter";

export function ReadingCanvas({ bookId, chapter }: { bookId: BookId; chapter: BookChapter }) {
  const { fontSize, readerTheme, readerFont, readerLineHeight, addHighlight, recordReadingMinutes, setProgress } = useReader();
  ...
  return <main className={`reading-canvas reading-canvas--${fontSize} reading-canvas--theme-${readerTheme} reading-canvas--font-${readerFont} reading-canvas--leading-${readerLineHeight}`}>
    ...
    <div className="reading-canvas__mobile-tools" aria-label="মোবাইল reading control"><BookmarkButton chapterId={chapterKey} /><ReaderPreferences compact /></div>
