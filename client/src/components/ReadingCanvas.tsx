/* জীবন-ড্যাশবোর্ড: chapter reader-এ web text ও verified PDF page range পাশাপাশি দেখায়। */
...
import { ArrowDown, Clock3, FileText, Quote } from "lucide-react";
...
export function ReadingCanvas({ bookId, chapter }: { bookId: BookId; chapter: BookChapter }) {
  ...
  const pageRange = getBookDefinition(bookId).chapters.find((item) => item.id === chapter.id);
  useEffect(() => {
  ...
  return <main className={`reading-canvas reading-canvas--${fontSize}`}>
    ...
    <div className="reading-canvas__mobile-note"><NotePanel noteKey={chapterNoteStorageKey(bookId, chapter.id)} /></div><footer className="reading-canvas__footer"><ArrowDown className="size-4" /> নিচে স্ক্রল করলে তোমার অগ্রগতি সেভ হবে</footer>
  </main>;
}
